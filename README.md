# AI 漫畫翻譯 / AI Manga Translator

內建瀏覽器截圖，經 Gemini、OpenAI 或本機 Ollama 翻譯後，把譯文疊回漫畫對話框。目前先做 Android APK。

An Android app with a built-in browser. It screenshots the current manga page, asks Gemini, OpenAI, or local Ollama to locate and translate speech bubbles, then overlays the translation on the page.
...
在 App 裡打開漫畫網站，等畫面載入後按「翻譯本頁」。程式會：
...
**瀏覽器（`BrowserScreen`）**  
主畫面。WebView 預設顯示說明首頁。網址列可貼漫畫網址；沒有 `http` 且像網域會自動加 `https://`，否則當 Google 搜尋。底部「翻譯本頁」截圖並翻譯。overlay 工具列可開關疊層、進入編輯、顯示原文、上下微調、清除本頁。返回鍵優先走網頁歷史。
...
**截圖結果（`ResultScreen`）**  
翻譯成功後可從 SnackBar 打開。顯示截圖與氣泡，可縮放、改譯文、切換原文／譯文。此頁編輯不會自動寫回網頁 overlay。

**書籤（`BookmarksScreen`）**  
書籤與「只有 overlay、沒有書籤」的頁面會一起列出。點列項開啟網址。可匯出 `ai-manga-library.json`，匯入時選合併或取代。刪書籤不會刪 overlay，除非另外刪 overlay。

### 翻譯器行為

三者共用 `PageTranslator`：輸入 JPEG 與長寬，輸出 `Bubble` 列表；另提供 `ping` 測連線。
...
`x,y` 是框左上角相對整張圖的比例。擬聲字可略過。解析時若模型給像素而非 0–1，會依圖寬高換算。
...
- 截圖會送到你設定的模型供應商（或本機 Ollama）。請自行遵守漫畫網站條款與著作權。
...
Open a manga site inside the app, wait for the page to load, then tap **Translate this page**. The app will:

1. Capture the current WebView screenshot
2. Send the image to the vision model you configured
3. Parse bubble coordinates and translations from the model JSON
...
All application code, architecture, UI, translation pipeline, and this documentation in this repository were created and revised by Cursor AI in chat. A human provided requirements, verified results, and published the project. If you fork or reuse this project, please keep this statement and the GPL license.
...
- Page overlay: translations sit on speech bubbles and follow scroll
...
- Font fitting: wrap at the configured minimum size first; shrink only if the bubble is still too small
- API keys stay in on-device `SharedPreferences`; this project has no backend that collects them

### How it works

```
Manga page (InAppWebView)
        │
   ...
   Tap "Translate this page"
        ...
        ├─ Parse bubbles and map them onto page-scroll coordinates
...
lib/
  ...
  models/
    ...
    overlay_page.dart       Saved overlay page and bubbles
  ...
  screens/
    ...
    result_screen.dart      Last screenshot with tappable bubbles
  ...
  services/
    ...
    bubble_parser.dart      Extract bubbles from model JSON
  ...
  widgets/
    ...
    bubble_text.dart        Bubble typography
...
Main screen. The WebView starts on an in-app help page. Paste a manga URL; missing `http` plus a dotted host becomes `https://`, otherwise it becomes a Google search. **Translate this page** captures and translates. The overlay toolbar toggles the layer, edit mode, original text, vertical nudge, and clear. The system back button prefers WebView history.
...
**Result (`ResultScreen`)**  
Opened from the success snackbar. Shows the screenshot and bubbles with pinch-zoom, edit, and original/translated toggle. Edits here are not written back to the page overlay.

**Bookmarks (`BookmarksScreen`)**  
Lists bookmarks together with overlay-only pages. Tap a row to open the URL. Export writes `ai-manga-library.json`. Import can merge or replace. Deleting a bookmark keeps the overlay unless you delete the overlay separately.

### Translators

All three implement `PageTranslator`: JPEG plus dimensions in, `Bubble` list out, plus `ping`.
...
`x,y` is the top-left of the box as a fraction of the full image. SFX may be skipped. If the model returns pixels instead of 0–1 ratios, the parser converts using image width and height.
...
3. Open a manga site in the address bar; switch to desktop mode if needed
4. When the page is visible, tap **Translate this page**
5. Translations overlay the page; tap a bubble to edit
...
- Screenshots are sent to the model provider you configure (or to local Ollama). Follow the manga site's terms and copyright law.
